...
class TestGroupTree(unittest.TestCase):
    def setUp(self):
        ...
        self.schedule = sunbeam.parse_schedule(norne, self.es)

    def test_group(self):
        gr = self.schedule.group(timestep=2)['PROD']
    ...
    def test_timestep_groups(self):
        total = 0
        for group in self.schedule.groups(timestep=3):
            for child in group.children:
                self.assertIsNotNone(child.name)
                total += 1
        self.assertEqual(13, total)

        group = self.schedule.group(timestep=3)['PROD']
